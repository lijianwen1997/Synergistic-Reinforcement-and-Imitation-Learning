...
def evaluate_policy_old(
    ...
    if reward_threshold is not None:
        for i in range(len(trajectory_l)):
            ...
            if episode_rewards[i]>=reward_threshold and len(trajectory_l[i].obs)<370:
                trajectory_good.append(trajectory_l[i])

            elif episode_rewards[i]<reward_threshold/2:
                trajectory_bad.append(trajectory_l[i])

        #assert mean_reward > reward_threshold, "Mean reward below threshold: " f"{mean_reward:.2f} < {reward_threshold:.2f}"
    #breakpoint()
    if return_episode_rewards:
        return episode_rewards, episode_lengths,trajectory_good,trajectory_bad
    return mean_reward, episode_rewards,trajectory_good,trajectory_bad
